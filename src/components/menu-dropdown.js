...
const DropdownContainer = ({ isMobile, open, transitionType, children }) => {
  ...
  return (
    ...
    >
      ...
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1, boxShadow: open ? "0px 4px 10px rgba(0, 0, 0, 0.25)" : "0px 4px 10px rgba(0, 0, 0, 0.1)" }}
...
export const MenuDropdown = ({ title, items, path }) => {
  ...
  return (
    ...
    >
      ...
      >
        <p>{title}</p>
        {isMobile && <motion.svg xmlns="http://www.w3.org/2000/svg" width="14" height="10" viewBox="0 0 14 10" fill="none" animate={open ? { rotateZ: 180 } : { rotateZ: 0 }}>
          <path d="M1.5 1.5L7 8.5L12.5 1.5" stroke="black" strokeWidth="1.5" strokeLinecap="square" strokeLinejoin="round"/>
        </motion.svg>}
      </div>
      <AnimatePresence>
        {open && (
